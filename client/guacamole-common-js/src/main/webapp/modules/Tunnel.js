...
var bitrateUp = null;
var bitrateDown = null;

var bytesTempUp = 0;
var bytesTempDown = 0;
var bytesTotal = 0;

function formatBytesToBits(bytes, decimals) {
    decimals = decimals || 2;
    var bits = bytes * 8;
    ...
    return new Array(parseFloat((bits / Math.pow(k, i)).toFixed(dm)), sizes[i]);
}

function updateBitrate(interval) {
    interval = interval || 1;
    var up = bytesTempUp / interval;
    var down = bytesTempDown / interval;
    resultUp = formatBytesToBits(up);
    resultDown = formatBytesToBits(down);
    bitrateUp = "<b>" + resultUp[0] + "</b> " + resultUp[1] + "/s";
    bitrateDown = "<b>" + resultDown[0] + "</b> " + resultDown[1] + "/s";
    bytesTempUp = 0;
    bytesTempDown = 0;
    document.getElementById("bitrateUp").innerHTML = bitrateUp;
    document.getElementById("bitrateDown").innerHTML = bitrateDown;
}
...
Guacamole.WebSocketTunnel = function(tunnelURL) {
    ...
    this.sendMessage = function(elements) {
        ...
        var sizeInBytes = new Blob([message]).size;
        bytesTotal += sizeInBytes; 
        bytesTempUp += sizeInBytes;
    ...
    this.connect = function(data) {
        ...
        var temp = function() { updateBitrate(3); };
        var bitrateIntervalId = setInterval(temp, 3000);
        ...
        socket.onclose = function(event) {
            ...
            var bytes = bytesTotal / diff;
            close_tunnel(new Guacamole.Status(parseInt(event.reason), event.reason));
            clearInterval(bitrateIntervalId);
            bitrateUp = "<b>0</b> b/s";
            bitrateDown = "<b>0</b> b/s";
            document.getElementById("bitrateUp").innerHTML = bitrateUp;
            document.getElementById("bitrateDown").innerHTML = bitrateDown;
        };
