...
int guaclog_interpret(const char* path, const char* out_path,
        bool force, int interval) {
    ...
    /* Create second filename for _mouse */
    char out_path_mouse[4102];
    strcpy(out_path_mouse, out_path);
    /* Remove old '.txt' */
    out_path_mouse[strlen(out_path) - 4] = '\0';
    strcat(out_path_mouse, "_mouse.txt");
    /* NOTE: Probably a better way to get the mouse log filename. Maybe
     * pass it through all layers from guaclog.c, akin to 'path'.
     */


    /* Allocate input state for interpreting process */
    guaclog_state* state = guaclog_state_alloc(out_path, out_path_mouse,
            interval);
    if (state == NULL) {
        close(fd);
        return 1;
