...
int main(int argc, char* argv[]) {
    ...
    /* Parse arguments */
    int opt;
    while ((opt = getopt(argc, argv, "s:r:i:f")) != -1) {
        ...
        /* -i: Interval (seconds) */
        if (opt == 'i') {
            if (guacenc_parse_int(optarg, &interval)) {
                guaclog_log(GUAC_LOG_ERROR, "Invalid interval.");
    ...
    for (i = optind; i < argc; i++) {
        ...
        if (guaclog_interpret(path, out_path, force, interval)) {
            failures++;
