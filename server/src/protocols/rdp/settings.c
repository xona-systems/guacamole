...
enum RDP_ARGS_IDX {
    ...
    /**
     * "true" if desktop should span across multiple monitors, "false" if only
     * the primary monitor should be used.
     */
    IDX_ENABLE_MONITOR_SPANNING,
...
guac_rdp_settings* guac_rdp_parse_args(guac_user* user,
    ...
    settings->monitor_spanning_enabled = 
        guac_user_parse_args_boolean(user, GUAC_RDP_CLIENT_ARGS, argv,
                IDX_ENABLE_MONITOR_SPANNING, 0);
...
void guac_rdp_push_settings(guac_client* client,
    ...
    rdp_settings->SpanMonitors = guac_settings->monitor_spanning_enabled;
